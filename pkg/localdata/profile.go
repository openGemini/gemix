...
func InitProfile() *Profile {
	...
	case DefaultGemixHome != "":
		profileDir = DefaultGemixHome
	default:
		u, err := user.Current()
		if err != nil {
...
func (p *Profile) GetComponentInstalledVersion(component string, ver utils.Version) (utils.Version, error) {
	...
	if len(versions) == 0 {
		return "", errors.Errorf("component not installed, please try `gemix install %s` to install it", component)
	}
	...
	if ver.String() != utils.NightlyVersionAlias {
		...
		return "", errors.Errorf("component(nightly) not installed, please try `gemix install %s:nightly` to install it", component)
	}

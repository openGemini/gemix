...
func TestCASign(t *testing.T) {
	...
	csr, err := privKey.CSR("gemix", "testing-cn",
		[]string{
			"ts-sql",
			"ts-sql.server.local",
		}, []string{
	...
	assert.Equal(t, []string{pkixOrganizationalUnit, "gemix"}, cert.Subject.OrganizationalUnit)
	assert.Equal(t, "testing-cn", cert.Subject.CommonName)
	assert.Equal(t, []string{"ts-sql", "ts-sql.server.local"}, cert.DNSNames)

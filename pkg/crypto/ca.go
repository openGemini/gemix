...
func ReadCA(clsName, certPath, keyPath string) (*CertificateAuthority, error) {
	...
	if err != nil {
		return nil, errors.WithMessagef(err, "error reading CA private key for %s", clsName)
	}
	...
	case "RSA PRIVATE KEY":
		...
		if err != nil {
			return nil, errors.WithMessagef(err, "error decoding CA private key for %s", clsName)
		}
	...
	if err != nil {
		return nil, errors.WithMessagef(err, "error reading CA certificate for %s", clsName)
	}
	...
	if err != nil {
		return nil, errors.WithMessagef(err, "error decoding CA certificate for %s", clsName)
	}

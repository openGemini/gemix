...
import (
	...
	"github.com/openGemini/gemix/pkg/base52"
	"github.com/openGemini/gemix/pkg/crypto/rand"
...
func decodeCommandArgs(args []string) ([]string, error) {
	...
	for _, arg := range args {
		...
		if err != nil {
			return nil, errors.WithMessage(err, "failed on decode the command line of audit log")
		}
...
func OutputAuditLog(dir, fileSuffix string, data []byte) error {
	...
	if err != nil {
		return errors.WithMessage(err, "create audit log")
	}
	defer f.Close()

	args := encodeCommandArgs(os.Args)
	if _, err := f.Write([]byte(strings.Join(args, " ") + "\n")); err != nil {
		return errors.WithMessage(err, "write audit log")
	}
...
func ShowAuditLog(dir string, auditID string) error {
	...
	if err != nil {
		return errors.WithMessagef(err, "unrecognized audit id '%s'", auditID)
	}
...
func DeleteAuditLog(dir string, retainDays int, skipConfirm bool, displayMode string) error {
	...
	if displayMode == "json" {
		...
		if !skipConfirm {
			if err := gui.PromptForConfirmOrAbortError("Do you want to continue? [y/N]:"); err != nil {
				return err
			}
		}
	}
